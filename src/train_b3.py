...
def train():
    ...
    b2_checkpoint_path = "checkpoints/b2/best.pth"

    if os.path.exists(b2_checkpoint_path):
        checkpoint = torch.load(b2_checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint["model_state_dict"])
        print(f"Initialized B3 from B2 checkpoint epoch {checkpoint['epoch']}")
        print(f"B2 val total loss: {checkpoint['val_total_loss']:.6f}")
    else:
        print("B2 checkpoint not found. Training B3 from scratch.")
    ...
    for epoch in range(1, config["training"]["epochs"] + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(
                f"Best B3 model saved at epoch {epoch} "
