...
def compute_loss(pred, clear, gate, l1_fn, lpips_fn, clip_model, config):
    ...
    return total, l1.item(), perceptual.item(), semantic.item(), gate_reg.item()
...
def validate(model, val_loader, l1_fn, lpips_fn, clip_model, config, device):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            loss, l1_value, lpips_value, clip_value, gate_reg_value = compute_loss(
                pred,
                clear,
                gate,
                l1_fn,
...
def save_samples(model, val_loader, device, result_dir, epoch):
    ...
    sample_path = os.path.join(result_dir, f"epoch_{epoch:03d}_samples.png")
    save_image(comparison, sample_path, nrow=4)

    # Upsample gate maps for visualization
    gate_vis = F.interpolate(
        gate[:4],
        size=(256, 256),
        mode="bilinear",
        align_corners=False
    ...
    gate_path = os.path.join(result_dir, f"epoch_{epoch:03d}_gates.png")
    save_image(gate_vis, gate_path, nrow=4)


def load_partial_b3_weights(model, checkpoint_path, device):
    if not os.path.exists(checkpoint_path):
        print("B3 checkpoint not found. Training B6 from scratch.")
        return

    checkpoint = torch.load(checkpoint_path, map_location=device)
    b3_state = checkpoint["model_state_dict"]
    b5_state = model.state_dict()

    loaded_keys = []
    skipped_keys = []

    for key, value in b3_state.items():
        if key in b5_state and b5_state[key].shape == value.shape:
            b5_state[key] = value
            loaded_keys.append(key)
        else:
            skipped_keys.append(key)

    model.load_state_dict(b5_state)

    print(f"Initialized B6 partially from B3 checkpoint epoch {checkpoint['epoch']}")
    print(f"Loaded compatible keys: {len(loaded_keys)}")
    print(f"Skipped keys: {len(skipped_keys)}")

def freeze_backbone_train_semantic_modules(model):
    """
    Freeze UNet encoder/decoder/restoration backbone.
    Train only gated cross-attention module.
    ...
    for param in model.scaled_gated_cross_attention.parameters():
        param.requires_grad = True

    print("Frozen backbone. Training only gated cross-attention module.")
...
def train():
    config = load_config("configs/b7_scaled_gated_attention.yaml")
    ...
    freeze_backbone_train_semantic_modules(model)
    ...
    if os.path.exists(best_path):
        try:
            ...
            print(f"Best previous B7 val loss: {best_val_loss:.6f}")
        except RuntimeError:
            print("Existing B7 best checkpoint is corrupted. Ignoring it.")
    ...
    try:
        if os.path.exists(resume_path):
            ...
            print(f"Resumed B7 from last checkpoint epoch {checkpoint['epoch']}")
            ...
            print(f"Resumed B7 from best checkpoint epoch {checkpoint['epoch']}")

        else:
            print("No B7 checkpoint found. Starting from initialized B3 weights.")

    except RuntimeError:
        print("Corrupted B7 last checkpoint detected. Falling back to best checkpoint.")
        ...
        if os.path.exists(best_path):
            ...
            print("No valid B7 checkpoint found. Starting from initialized B3 weights.")
    ...
    if not os.path.exists(train_log_path) or start_epoch == 1:
        with open(train_log_path, "w") as f:
            f.write(
                "epoch,train_total_loss,train_l1,train_lpips,train_clip,train_gate_mean,"
                "val_total_loss,val_l1,val_lpips,val_clip,val_gate_mean\n"
            )
    else:
        print("Existing B7 log file found. Appending resumed logs.")
    ...
    for epoch in range(start_epoch, config["training"]["epochs"] + 1):
        ...
        for batch in progress_bar:
            ...
            if torch.isnan(loss):
                print("NaN loss detected. Stopping B7 training.")
                return
        ...
        avg_train_gate = total_gate_mean / n_train
        avg_train_gate_reg = total_gate_reg / n_train
        ...
        print(
            ...
            f"Train Gate: {avg_train_gate:.6f} | "
            f"Train GateReg: {avg_train_gate_reg:.6f} | "
            f"Val Total: {avg_val_loss:.6f} | "
            f"Val L1: {avg_val_l1:.6f} | "
            f"Val LPIPS: {avg_val_lpips:.6f} | "
            f"Val CLIP: {avg_val_clip:.6f} | "
            f"Val Gate: {avg_val_gate:.6f}"
        )

        with open(train_log_path, "a") as f:
            f.write(
                f"{epoch},{avg_train_loss},{avg_train_l1},{avg_train_lpips},{avg_train_clip},{avg_train_gate},{avg_train_gate_reg},"

                f"{avg_val_loss},{avg_val_l1},{avg_val_lpips},{avg_val_clip},{avg_val_gate}\n"
            )
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(
                f"Best B7 model saved at epoch {epoch} "
