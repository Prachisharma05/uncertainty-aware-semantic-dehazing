...
def evaluate():
    ...
    print(f"Loaded B5 checkpoint from epoch {checkpoint['epoch']}")
    ...
    print(f"Validation Gate Mean: {checkpoint['val_gate_mean']:.6f}")
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Evaluating B5"):
    ...
    results = {
        "model": "B5_UNet_GatedCrossAttention",
        ...
        "val_gate_mean": checkpoint["val_gate_mean"],
    ...
    os.makedirs("results/b5", exist_ok=True)

    df = pd.DataFrame([results])
    df.to_csv("results/b5/b5_metrics.csv", index=False)

    print("\n--- B5 Evaluation Results ---")
    print(df)
