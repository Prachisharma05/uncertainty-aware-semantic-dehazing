...
def evaluate():
    ...
    print(f"Loaded B4 checkpoint from epoch {checkpoint['epoch']}")
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Evaluating B4"):
    ...
    df.to_csv("results/b4/b4_metrics.csv", index=False)

    print("\n--- B4 Evaluation Results ---")
    print(df)
