...
B6_NH_CKPT = "checkpoints/b6_nh/best.pth"

OUT_CSV = "results/nh_haze/metrics/nh_haze_b3_b6_b6nh_metrics.csv"

os.makedirs("results/nh_haze/metrics", exist_ok=True)
...
def main():
    ...
    print("NH-HAZE images:", len(hazy_files))
    ...
    b6_nh = load_b6(B6_NH_CKPT, "B6_NH")

    results = []

    results.append(
        evaluate_model(b3, "B3", hazy_files, psnr_metric, ssim_metric, lpips_metric)
    )

    results.append(
        evaluate_model(b6, "B6", hazy_files, psnr_metric, ssim_metric, lpips_metric)
    )

    results.append(
        evaluate_model(b6_nh, "B6_NH_Finetuned", hazy_files, psnr_metric, ssim_metric, lpips_metric)
    )

    df = pd.DataFrame(results)
    df.to_csv(OUT_CSV, index=False)

    print("\n===== NH-HAZE Final Evaluation =====")
    print(df)
