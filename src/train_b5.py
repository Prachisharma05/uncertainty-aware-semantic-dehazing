...
def load_partial_b3_weights(model, checkpoint_path, device):
    if not os.path.exists(checkpoint_path):
        print("B3 checkpoint not found. Training B5 from scratch.")
        return
    ...
    print(f"Initialized B5 partially from B3 checkpoint epoch {checkpoint['epoch']}")
    print(f"Loaded compatible keys: {len(loaded_keys)}")
    print(f"Skipped keys: {len(skipped_keys)}")


def train():
    config = load_config("configs/b5_gated_cross_attention.yaml")
    ...
    if os.path.exists(best_path):
        try:
            ...
            print(f"Best previous B5 val loss: {best_val_loss:.6f}")
        except RuntimeError:
            print("Existing B5 best checkpoint is corrupted. Ignoring it.")
    ...
    try:
        if os.path.exists(resume_path):
            ...
            print(f"Resumed B5 from last checkpoint epoch {checkpoint['epoch']}")
            ...
            print(f"Resumed B5 from best checkpoint epoch {checkpoint['epoch']}")

        else:
            print("No B5 checkpoint found. Starting from initialized B3 weights.")

    except RuntimeError:
        print("Corrupted B5 last checkpoint detected. Falling back to best checkpoint.")
        ...
        if os.path.exists(best_path):
            ...
            print("No valid B5 checkpoint found. Starting from initialized B3 weights.")
    ...
    if not os.path.exists(train_log_path) or start_epoch == 1:
        ...
        print("Existing B5 log file found. Appending resumed logs.")
    ...
    for epoch in range(start_epoch, config["training"]["epochs"] + 1):
        ...
        for batch in progress_bar:
            ...
            if torch.isnan(loss):
                print("NaN loss detected. Stopping B5 training.")
                return
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(
                f"Best B5 model saved at epoch {epoch} "
